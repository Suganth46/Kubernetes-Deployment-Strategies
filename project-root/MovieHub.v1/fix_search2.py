...
content = re.sub(
    r'<div class="flex w-full flex-col gap-3 sm:flex-row md:w-auto">\s*<input\s*id="movie-search"\s*type="search"\s*placeholder="Search year, title, or genre"\s*class="[^"]+"\s*/>\s*<button',
    new_search,
    content
)
...
content = re.sub(
    r'const searchInput = document\.getElementById\("movie-search"\);\s*const clearButton = document\.getElementById\("clear-filters"\);',
    new_script,
    content
)

Path("static/index.html").write_text(content, encoding="utf-8")
print("Search fixed for real")
